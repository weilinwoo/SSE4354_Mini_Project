...
public class verifyOTPServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String tmpOTP = request.getParameter("otp");//OTP entered by customer
		int otp = Integer.parseInt(tmpOTP);
		
		if(validOTP(id,otp)) {
			//update balance if OTP is valid 
			updateBalance(id,payment);
			request.getRequestDispatcher("statusPayment.jsp").forward(request, response);
		}else {
	...
	//invoke web services to verify OTP (invoke verifyOTP method in Bank A's HandleOtpEJB.java)
